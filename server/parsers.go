...
func ParseMultipartContent(call *Call, p *multipart.Part, b []byte) {
		...
		if err := json.Unmarshal(b, &f); err == nil {
			...
			case []any:
				for _, f := range v {
					...
					if call.Frequency == 0 && freq.Frequency > 0 {
						call.Frequency = freq.Frequency
					}
				}
			}
		}
	...
	case "siteId":
		if i, err := strconv.Atoi(string(b)); err == nil && i > 0 {
			call.Meta.SiteId = uint64(i)
		}

	case "source":
		if i, err := strconv.Atoi(string(b)); err == nil {
			// For v6 compatibility, create a simple unit
			unitRef := uint(i)
			call.Units = append(call.Units, CallUnit{
				Offset:  0,
				UnitRef: unitRef,
	...
	case "system", "systemId":
		if i, err := strconv.Atoi(string(b)); err == nil && i > 0 {
			call.SystemId = uint(i)
			call.Meta.SystemRef = uint(i)
			// Also set Meta.SystemId so it can be used as the database primary key when auto-creating
			call.Meta.SystemId = uint64(i)
		}
...
func ParseTrunkRecorderMeta(call *Call, b []byte) error {
	...
	case float64:
		if v > 0 {
			call.SystemId = uint(v)
			call.Meta.SystemRef = uint(v)
			// Also set Meta.SystemId so it can be used as the database primary key when auto-creating
			call.Meta.SystemId = uint64(v)
		}
	}
