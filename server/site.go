	...
	for _, site := range sites.List {
		...
		if count == 0 {
			if site.Id > 0 {
				...
				query = fmt.Sprintf(`INSERT INTO "sites" ("siteId", "label", "order", "siteRef", "systemId") VALUES (%d, '%s', %d, %d, %d)`, site.Id, escapeQuotes(site.Label), site.Order, site.SiteRef, systemId)
			} else {
